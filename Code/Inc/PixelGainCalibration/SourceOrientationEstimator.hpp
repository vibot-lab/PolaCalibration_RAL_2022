 ...
 * blocks of four elements, arranged in a matrix of 2 x 2 elements. This pattern
 * is repeated over all the image, in both directions, horizontal and vertical.
...
public:
    ...
    */
     ...
     *      S = [K * A  K * cos(2*alpha) K * sin(2*alpha)] is the stokes vector of the input light
     ...
     *
     *                         C^+ * I = S
     *  If we write the solution S = [s0 s1 s2] = [K * A  K * cos(2*alpha) K * sin(2*alpha)],
