 ...
 * calibration matrix.
 * For a fixed, uniform light, when all the pixels are exposed, we should
 ...
 * choose to take the average, the median or the maximum value to estimate these light
 * parameters, and we consider them as our true values to compute Ti and Pi for each pixel.
 *
 ...
 * one, we change the boolean value when we call the function computeCalibrationMatrices.
*/
...
public:
    ...
    */
     ...
     * @arg horAxis: Vector the values of the horizontal axis at which each sample has been taken.
     * This vector corresponds to the different polarization angles.
     * @arg avgImgs: Vector with the averaged images at each value of the horizontal axis vector.
     * @arg useGtAngles: If true, the given angles are used for the calibration. If false,
     *  the AoLP estimator is used.
    */
    void loadData(const std::vector<int> camFilterOrient,
        const std::vector<double>& horAxis,
        const std::vector<cv::Mat>& avgImgs,
        bool useGtAngles);

    /// \brief getCalibrationDoP: Get the degree of polarization of the light used for the calibration
    std::vector<double> getCalibrationDoP() const { return _lightDoP; }

    /// \brief getCalibrationS0: Get the intensity of the light used for the calibration
    ...
    */
    ...
    /**
     * @brief getSamples: Get a vector with all the sampled images retrieved
     * from the calibration experiment.
    */
    const std::vector<cv::Mat>& getSamples() const {return _rawSamples;}

    /**
     * @brief computeCalibrationMatrices: Generate the matrices of the different
    ...
    */
    void computeCalibrationMatrices(bool useSuperPixel);

    /// \brief isDataLoaded: Check if the experiment data has been loaded.
    bool isDataLoaded() const;

    /// \brief areCalibrationComputed: Check if the calibration matrices have been already computed.
     ...
     *  If this has not been done previously, the input raw image is returned without correction.
     *
     * @arg rawImg: Input raw image from the camera.
     * @arg correct: If True, the calibration correction is applied. If False, the
     *   input image is copied into the output image.
...
private:
     ...
     * color filter. This function works only for the BayerRG pattern.
     *
    ...
    */
     ...
     *  explained for the function computeCalibrationMatrices.
     *
    ...
    */
     ...
     * been considered, the median value will be computed and stored in _lightS0
