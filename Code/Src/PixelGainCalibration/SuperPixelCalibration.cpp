...
// Custom includes
#include "PixelGainCalibration/SourceOrientationEstimator.hpp"
...
void SuperPixelCalibration::loadData(const std::vector<int> camFilterOrient,
    const std::vector<double>& angles,
    const std::vector<cv::Mat>& avgImgs,
    bool useGtAngles)
{
    std::vector<double> usedAngles;
    std::vector<cv::Mat> usedImgs;
    int estimatorROISize = 50;
    if (useGtAngles)
    {
        usedAngles = angles;
        usedImgs = avgImgs;
    ...
    {
        SourceOrientationEstimator anglesEstimator(camFilterOrient, estimatorROISize);
        usedAngles.clear();
        usedImgs.clear();
        std::vector<std::pair<double, cv::Mat>> mergedInfo;

        // We convert the images into a vector of pairs (angles, images)
        std::transform(
            avgImgs.begin(),
            avgImgs.end(),
            std::back_inserter(mergedInfo),
            [anglesEstimator](const cv::Mat& img)
            {
                return std::pair<double, cv::Mat>(anglesEstimator.getLightSourceOrientation(img), img);
            }
        );

        std::cout << "Angles comparison:" << std::endl;
        for(unsigned int i = 0; i < mergedInfo.size(); i++)
        {
            std::cout << "GT angle: " << angles[i] << " - Estimated angle: " << mergedInfo[i].first << " - Corrected angle: " << mergedInfo[i].first - mergedInfo[0].first << std::endl;
        }

        // We sort the pairs (angles, images), based on the angles
        std::sort(mergedInfo.begin(),
            mergedInfo.end(),
            [](const std::pair<double, cv::Mat> &v1, const std::pair<double, cv::Mat> &v2){ return v1.first < v2.first; });

        // We split the angles and the images into two vectors
        for(auto& pair : mergedInfo)
        {
            usedAngles.push_back(pair.first);
            usedImgs.push_back(pair.second);
        }
    }
    ...
    {
        reset();
        _anglesVector = usedAngles;

        cv::merge(usedImgs, _allImagesSamples);
        cv::split(_allImagesSamples, _rawSamples);
        std::cout << "Resulting images size: (" << _allImagesSamples.rows << ", " << _allImagesSamples.cols << ", " << _allImagesSamples.channels() << ")"<< std::endl;
    }
}
...
void SuperPixelCalibration::computeCalibrationMatrices(bool useSuperPixel)
{
    if (!_allImagesSamples.empty())
    {
        ...
        for(int i = 0; i < rows; i += deltaRows)
        {
            for(int j = 0; j < cols; j += deltaCols)
            {
                ...
                {
                    cv::Mat(1, channels, CV_64FC1, _allImagesSamples.ptr(rowsIdx[r], colsIdx[r])).copyTo(samples.row(r));
                }
...
void SuperPixelCalibration::correctImage(cv::Mat& rawImg, bool correct, cv::Mat& output)
{
    if (correct && !_calibrationMatrix.empty())
    {
        // Sanity check
